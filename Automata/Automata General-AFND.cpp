...
int main()
{
    ...
    set<int> estAceptacion = {2}; // Estado de aceptacion
    set<pair<int, char>> transiciones[MaxDim];

    // Definición de transiciones
    transiciones[0].insert({1, '\0'}); // Transicion instantánea de 0 a 1
    transiciones[1].insert({1, 'a'}); // Transicion de 1 a 1 con 'a'
    transiciones[1].insert({2, 'b'}); // Transici0n de 1 a 2 con 'b'
